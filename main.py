...
def usage():
    ...
    print("Usage: photos.py /path/to/folder")
    sys.exit(1)
...
def get_input_folder():
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) > 2:
            print("⚠️  Warning: Only the first folder path will be processed.")
    else:
        print("✅ Please enter the path to your folder:")
    ...
    for forbidden in forbidden_dirs:
        if forbidden == input_folder or forbidden in input_folder.parents:
            print(f"❌ Error: The path '{input_folder}' is a protected system directory.")
            usage()
    ...
    for path in root_folder.rglob('*'):
        if path.is_file() and not is_excluded(path):
            try:
                ...
                print(f"❌ Error generating checksum for {path}: {e}")
    return checksums
        ...
        if len(file_list) > 1:
            ...
            for duplicate in file_list[1:]:
                try:
                    ...
                    print(f"❌ Error deleting duplicate {duplicate}: {e}")
...
def rename_files(target_folder, prefix):
    ...
    if target_folder.name in [RANDOM_FOLDER_NAME, SCREENSHOTS_FOLDER_NAME]:
        print(f"⏩ Skipping renaming for folder: {target_folder.name}")
    ...
    for idx, file in enumerate(files, 1):
        ...
        if file.suffix.lower() not in (".jpg", ".jpeg", ".bmp", ".tiff", ".tif", ".psd", ".svg", ".ico", ".jfif", ".pjpeg", ".pjp", ".avif", ".apng", ".heic", ".heif"):
...
def process_heic_image(path):
    ...
    try:
        ...
        print(f"✅ Processed HEIC: {path.name} - {width}x{height}")
        return width, height
    except Exception as e:
        ...
        if path.is_file() and not is_excluded(path):
            ...
            try:
                if suffix in (".heic", ".heif"):
                    ...
                    if width and height:
                        if height > width:
                            target_folder_name = PORTRAIT_FOLDER_NAME  # Updated
                            counts["Portrait"] += 1
                        else:
                            target_folder_name = LANDSCAPE_FOLDER_NAME  # Updated
                            counts["Landscape"] += 1
                        folder = dynamic_folders.setdefault(target_folder_name, root_folder / target_folder_name)
                        if not folder.exists():
                            folder.mkdir(parents=True, exist_ok=True)
                        shutil.move(str(path), folder / path.name)
                        print(f"✅ Moved HEIC to {target_folder_name}: {path.name}")

                elif suffix in (".jpg", ".jpeg", ".bmp", ".tiff", ".tif", ".psd", ".svg", ".ico", ".jfif", ".pjpeg", ".pjp", ".avif", ".apng"):
                    with Image.open(path) as img:
                        width, height = img.size
                        print(f"✅ Processed Image: {path.name} - {width}x{height}")
                        if height > width:
                            target_folder_name = PORTRAIT_FOLDER_NAME  # Updated
                            counts["Portrait"] += 1
                        else:
                        ...
                        if not folder.exists():
                            folder.mkdir(parents=True, exist_ok=True)
                        shutil.move(str(path), folder / path.name)
                        print(f"✅ Moved Image to {target_folder_name}: {path.name}")

                elif suffix == ".png":
                    folder = dynamic_folders.setdefault(SCREENSHOTS_FOLDER_NAME, root_folder / SCREENSHOTS_FOLDER_NAME)
                    if not folder.exists():
                        folder.mkdir(parents=True, exist_ok=True)
                    shutil.move(str(path), folder / path.name)
                    print(f"✅ Moved PNG (Screenshot): {path} to {folder}")

                elif suffix in (".gif", ".webp"):
                    folder = dynamic_folders.setdefault(GIF_FOLDER_NAME, root_folder / GIF_FOLDER_NAME)
                    if not folder.exists():
                        folder.mkdir(parents=True, exist_ok=True)
                    shutil.move(str(path), folder / path.name)
                    print(f"✅ Moved GIF: {path} to {folder}")

                elif suffix in (".dng", ".raw", ".nef", ".cr2", ".cr3", ".arw", ".orf", ".rw2", ".raf", ".srw", ".kdc"):
                    folder = dynamic_folders.setdefault(PRORAW_FOLDER_NAME, root_folder / PRORAW_FOLDER_NAME)
                    if not folder.exists():
                        folder.mkdir(parents=True, exist_ok=True)
                    shutil.move(str(path), folder / path.name)
                    print(f"✅ Moved ProRaw: {path} to {folder}")

                else:
                    folder = dynamic_folders.setdefault(RANDOM_FOLDER_NAME, root_folder / RANDOM_FOLDER_NAME)
                    if not folder.exists():
                        folder.mkdir(parents=True, exist_ok=True)
                    shutil.move(str(path), folder / path.name)
                    print(f"✅ Moved Misc: {path} to {folder}")
...
def clean_filenames(root_folder):
    ...
    for path in root_folder.rglob('*'):
        if path.is_file() and not is_excluded(path):
            ...
            cleaned_name = re.sub(r'\s+', ' ', cleaned_name).strip() # Replace multiple spaces with single
...
def print_summary_table(initial_count, deleted_count, remaining_count):
    ...
    print("\n")
    print(f"{TOP_LEFT}{HORIZONTAL*12}{T_DOWN}{HORIZONTAL*12}{T_DOWN}{HORIZONTAL*12}{T_DOWN}{HORIZONTAL*12}{TOP_RIGHT}")
    print(f"{VERTICAL} {'Extension':<10} {VERTICAL} {'PreCount':<10} {VERTICAL} {'Duplicates':<10} {VERTICAL} {'PostCount':<10} {VERTICAL}")
    print(f"{T_RIGHT}{HORIZONTAL*12}{CROSS}{HORIZONTAL*12}{CROSS}{HORIZONTAL*12}{CROSS}{HORIZONTAL*12}{T_LEFT}")
    ...
    for ext in sorted(initial_count.keys()):
        ...
        print(f"{VERTICAL} {ext:<10} {VERTICAL} {pre_count:<10} {VERTICAL} {dups:<10} {VERTICAL} {post_count:<10} {VERTICAL}")

    # Print the totals
    print(f"{T_RIGHT}{HORIZONTAL*12}{CROSS}{HORIZONTAL*12}{CROSS}{HORIZONTAL*12}{CROSS}{HORIZONTAL*12}{T_LEFT}")
    print(f"{VERTICAL} {'Totals:':<10} {VERTICAL} {total_pre:<10} {VERTICAL} {total_dups:<10} {VERTICAL} {total_post:<10} {VERTICAL}")
    print(f"{BOTTOM_LEFT}{HORIZONTAL*12}{T_UP}{HORIZONTAL*12}{T_UP}{HORIZONTAL*12}{T_UP}{HORIZONTAL*12}{BOTTOM_RIGHT}")
    print("\n")
...
def main():
    ...
    print("📊 Counting remaining files...")
    remaining_count = count_remaining_files(root_folder)

    # Ask for Portrait and Landscape prefixes
    portrait_prefix = input("Enter a prefix for Portrait photos (or press Enter to use 'Portrait'): ").strip() or "Portrait"
    landscape_prefix = input("Enter a prefix for Landscape photos (or press Enter to use 'Landscape'): ").strip() or "Landscape"

    # Sorting
    print("📂 Sorting files...")
    sort_counts = sort_files(root_folder, portrait_prefix, landscape_prefix)

    # Rename files in Portrait and Landscape folders (using user-provided prefixes)
    print("✍️ Renaming files in Portrait and Landscape folders...")
    for folder_name in [PORTRAIT_FOLDER_NAME, LANDSCAPE_FOLDER_NAME]:
        target_folder = root_folder / folder_name
        if target_folder.exists() and target_folder.is_dir():
            prefix = portrait_prefix if folder_name == PORTRAIT_FOLDER_NAME else landscape_prefix
